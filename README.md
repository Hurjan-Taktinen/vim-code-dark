...
(Additionally, if you don't want to or cannot use `t_Co`, you can `let g:konedark_term256=1`.)
...
Then copy a script from this (`vim-code-dark`) repository (`base16/templates/shell/scripts/base16-konedark.sh`) into `~/.config/base16-shell/scripts`.
...
Now start a new shell and type the following command: `base16_konedark`.
...
##### 3.3.3) PuTTY
PuTTY should actually support 256 colors, try following [steps on StackOverflow](http://superuser.com/questions/436910/emulate-256-colors-in-putty-terminal). If it does not work, run `base16/templates/putty/putty/base16-konedark.reg` to modify your registry, then run PuTTY and load `konedark` in the session list. This will modify your PuTTY terminal colors.
...
Italics in comments can be enabled by setting `g:konedark_italics = 1` _before_
setting the colorscheme in your `.vimrc` like so:
```
let g:konedark_italics = 1
colorscheme konedark
```
